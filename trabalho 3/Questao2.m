...
% Aplica a DPCM na imagem
initcodebook = [-1:.1:1];
for a = 1:x
    linha = double(refdpcm(a:a,1:y));
    [predictor,codebook,partition] = dpcmopt(linha,1,initcodebook);
    encoded = dpcmenco(linha,codebook,partition,predictor);
    decoded = dpcmdeco(encoded,codebook,predictor);
    refdpcm(a:a,1:y) = decoded;
end
